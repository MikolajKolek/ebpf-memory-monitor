...
use ebpf_common::vmlinux::mm_struct;
...
#[kprobe]
pub fn on_may_expand_vm(ctx: ProbeContext) -> u32 {
    let mm: Option<*const mm_struct> = ctx.arg(0);
    let npages: Option<c_ulong> = ctx.arg(2);

    if let Some(mm) = mm && let Some(npages) = npages {
